        ...
        private void LoadSampleRequestDocument(string SMPLNo, string SMPLModify)
        {
            ...
            if (SMPLNo != "")
            {
                ...
                if (arrSMPL.Length > 0)
                {
                    ...
                    sbSQL.Append("        SUBSTRING((SELECT GarmentParts + ', ' AS 'data()' FROM(SELECT GP.OIDGParts, GP.GarmentParts FROM SMPLRequestFabricParts AS SFBP INNER JOIN GarmentParts AS GP ON SFBP.OIDGParts = GP.OIDGParts AND SFBP.OIDSMPLFB = MKD.OIDSMPLDTStuff) AS FParts FOR XML PATH('')), 1, LEN((SELECT GarmentParts + ', ' AS 'data()' FROM(SELECT GP.OIDGParts, GP.GarmentParts FROM SMPLRequestFabricParts AS SFBP INNER JOIN GarmentParts AS GP ON SFBP.OIDGParts = GP.OIDGParts AND SFBP.OIDSMPLFB = MKD.OIDSMPLDTStuff) AS FParts FOR XML PATH(''))) -2) AS FabricParts, ");
